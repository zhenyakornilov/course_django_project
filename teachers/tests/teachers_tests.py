...
class TestStudentModelRelatedViews:
    ...
    def test_create_teacher_view(self, admin_client, create_teacher):
        ...
        response = admin_client.post('/create-teacher/', data={'subject': 'Math', 'first_name': 'Name',
    ...
    def test_all_teachers_view(self, admin_client, create_teacher):
        response = admin_client.get('/all-teachers/')
        assert response.status_code == 200
    ...
    def test_edit_teacher_view(self, admin_client, create_teacher):
        teacher = Teacher.objects.get(pk=1)
        response = admin_client.get(f'/edit-teacher/{teacher.pk}/')
        assert response.status_code == 200
        assertTemplateUsed(response, 'teachers/teacher_edit_form.html')

        response = admin_client.post(f'/edit-teacher/{teacher.pk}/',
    ...
    def test_delete_teacher_view(self, admin_client, create_teacher):
        teacher = Teacher.objects.get(pk=1)
        response = admin_client.post(f'/delete-teacher/{teacher.pk}/', follow=True)
        assert response.status_code == 200
